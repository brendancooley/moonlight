...
if __name__ == "__main__":
    ...
    st.write(f"Dollars available: ${400*n_teams - int(data_league_nfa['Salary'].sum())}")
    st.write(f"Implied rostered value: "
             f"${int(data_league_nfa['Value_Scaled'].sum())}")
    st.write(f"Implied free agent value: "
             f"${int(data_league_fa['Value_Scaled'].sum())}")
    ...
    left_cols = ["firstname", "lastname", "age", "Salary", "Value_Scaled", "Surplus"]
